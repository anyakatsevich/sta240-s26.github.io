...
subtitle: Due Thursday February 5 at 11:59 PM
format: html
draft: false
editor: 
  mode: source
...
In order to do this week's lab, regardless of whether you're using the containers or your local RStudio, you will need to install the `kgrams` package. Copy the following code into your console and run it:
...
change_temperature <- function(probs, temperature){
  new_probs <- (probs/max(probs)) ^ (1/temperature) # for numerical stability
...
Run this function and use the output together with the `sample` function to simulate a new word. **Print out that new word.**

::: {.callout-warning collapse="false"}
## Important
Make sure you (a) set a seed and (b) run the previous two code chunks (including the masked one on the template) before trying to use any commands that they use/reference.
:::

## Task 2 (we do it)

Use the `paste` function to append the word you simulated in Task 1 to the end of `context`. Then, starting from this updated context, go back to the beginning, generate a new conditional distribution, and use it to simulate a new word. **Print out that new word.**

## Task 3 (we do it)

Write a for loop that iterates the process in Tasks 1 and 2 in order to add **six** new words to the context. **Print out the new context (with the six additional words).**

::: callout-note
## This might be a tad slow.
The function `next_word_probs` is doing a lot of work under the hood, so in the upcoming tasks, do not be alarmed if it takes a few seconds/minutes to run. 
:::

## Task 4 (you do it)

Rerun your code five times to generate five different responses. **Print each one out.** How many of them would you consider *approximately* coherent?

::: {.callout-warning collapse="false"}
## Patience vs. time
Given the even more frequently repeated calls of `next_word_probs`, this may take a while. When you go to render the PDF, this part will take a few minutes. While it should take no longer than ten minutes under our suggestions, don't let this rendering time take over your life. **Fewer than five reruns is fine.**
:::
...
::: {.callout-warning collapse="false"}
## Patience vs. time
Again, you don't have to try ten different starting contexts. **Two or three will be fine.**
:::
...
::: {.callout-warning collapse="false"}
## Patience vs. time
Once more, **just a couple temperatures will be fine.**
:::
