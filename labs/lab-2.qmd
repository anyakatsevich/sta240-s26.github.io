---
title: "Lab 2"
subtitle: Due Thursday January 22 at 11:59 PM
editor: 
  mode: source
---
...
**IMPORTANT:** Run your simulation **at least 500,000** times -- the probability of the event is rather small.
...
- The event $A$ we care about is "the rooks are safe from one another".
- We know that a safe placement will have one rook per row. Thus a configuration of rooks is determined by 8 numbers: the position of the single rook in each of the 8 rows. There are 8 ways to place a single rook on the bottom row. There are only 7 ways to place the next rook in the next row, because we must avoid the column occupied by the first rook. Similarly, there are only 6 ways to place the next rook in the next row, because we must avoid the two columns occupied by the first two rooks. And so on. The counting principle gives $\#(A)=8\times7\times6\times\cdots\times2\times1=8!$. 
- So 
